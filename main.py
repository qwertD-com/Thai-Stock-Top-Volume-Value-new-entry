...
def get_gspread_client():
    ...
    if creds_json:
        ...
        raise EnvironmentError('GOOGLE_SHEETS_CREDENTIALS not found.')
...
def main():
    ...
    for d in historical_window:
        ...
        if not df_h.empty: hist_pool.update(df_h['Ticker'].tolist())

    # 5. Build Reports
    vol_df = get_top_20(today, 'volume')
    vol_report = [{'Rank': i+1, 'Ticker': r['Ticker'], 'Volume': r['Vol'], 'Value_MB': round((r['Vol']*r['Price'])/1e6, 2), 'Status': 'NEW ENTRY' if r['Ticker'] not in hist_pool else ''} for i, r in vol_df.reset_index().iterrows()]

    val_df = get_top_20(today, 'value')
    val_report = [{'Rank': i+1, 'Ticker': r['Ticker'], 'Price': r['Price'], 'Value_MB': round(r['Metric']/1e6, 2)} for i, r in val_df.reset_index().iterrows()]
    ...
    print(f'Sync Successful for {today.strftime("%Y-%m-%d")}')
